...
/**
 * @typedef {Object} qidain.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} qidain.HTTPCallback
...
/**
 * @typedef {function(Object, qidain.HTTPCallback): qidain.HTTPResponse} qidain.HTTPMethod
 */

/**
 * @typedef {Object} qidain.HttpClient
 * @property {qidain.HTTPMethod} get - 发送 GET 请求
 * @property {qidain.HTTPMethod} post - 发送 POST 请求
 * @property {qidain.HTTPMethod} put - 发送 PUT 请求
 * @property {qidain.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {qidain.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<qidain.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {qidain.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<qidain.HTTPResponse>}
 */
...
function runtimeCode() {
    function addDlCheckClientShow() {
        // 起点基于 dl_check_client_show 记录弹窗缓存
        let now = new Date().getTime().toString()
        localStorage.setItem('dl_check_client_show', now);
        console.log("update dl_check_client_show")
    }
    addDlCheckClientShow()
