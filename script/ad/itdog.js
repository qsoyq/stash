...
/**
 * @typedef {Object} itdog.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} itdog.HTTPCallback
...
/**
 * @typedef {function(Object, itdog.HTTPCallback): itdog.HTTPResponse} itdog.HTTPMethod
 */

/**
 * @typedef {Object} itdog.HttpClient
 * @property {itdog.HTTPMethod} get - 发送 GET 请求
 * @property {itdog.HTTPMethod} post - 发送 POST 请求
 * @property {itdog.HTTPMethod} put - 发送 PUT 请求
 * @property {itdog.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {itdog.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<itdog.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {itdog.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<itdog.HTTPResponse>}
 */
...
function removeAds(document) {
    ...
    // 广告图片
    Array.from(document.querySelectorAll('span[class="ad_tip"]')).forEach(e => {
        e.parentElement.remove()
    })
    Array.from(document.querySelectorAll('h5')).filter(h5 => h5.textContent === '赞助商').forEach(e => {
        e.parentElement.parentElement.parentElement.remove()
    })
    Array.from(document.querySelectorAll('div[class="gg_link"]')).forEach(e => { e.remove() })
    Array.from(document.querySelectorAll('div[class="footer mt-3"]')).forEach(e => { e.remove() })
    Array.from(document.querySelectorAll('div[class="lantern lantern_left gg_link"]')).forEach(e => { e.remove() })
    Array.from(document.querySelectorAll('div[class="lantern lantern_right gg_link"]')).forEach(e => { e.remove() })
}
    ...
    switch (getScriptType()) {
        case "response":
            ...
            if (ct && ct.includes("text/html") && body) {
                echo(`[script] [response]  ${$request.url}`)
