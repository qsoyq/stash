/** @namespace didi.integrated.marketing */

/**
 * @typedef {Object} didi.integrated.marketing.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} didi.integrated.marketing.HTTPCallback
...
/**
 * @typedef {function(Object, didi.integrated.marketing.HTTPCallback): didi.integrated.marketing.HTTPResponse} didi.integrated.marketing.HTTPMethod
 */

/**
 * @typedef {Object} didi.integrated.marketing.HttpClient
 * @property {didi.integrated.marketing.HTTPMethod} get - 发送 GET 请求
 * @property {didi.integrated.marketing.HTTPMethod} post - 发送 POST 请求
 * @property {didi.integrated.marketing.HTTPMethod} put - 发送 PUT 请求
 * @property {didi.integrated.marketing.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {didi.integrated.marketing.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<didi.integrated.marketing.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {didi.integrated.marketing.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<didi.integrated.marketing.HTTPResponse>}
 */
...
function removeAdsCode() {
    Object.defineProperty(window, '__INITIAL_STATE__', {
        set(value) {
            value.biz = null
            value.popup = null
            let multiPages = value?.xpub?.page?.scenes?.multiPages
            if(multiPages){
                for(let page_index=0; page_index<multiPages.length;page_index++){
                    let page = multiPages[page_index]
                    let layers = page.layers
                    if (layers){
                        for(let layer_index=0; layer_index<layers.length; layer_index++){
                            let layer = layers[layer_index]
                            if(layer?.props?.businessData?.general?.res_pos_data?.didi_applets_h5_hot_zone){
                                value.xpub.page.scenes.multiPages[page_index].layers[layer_index].props.businessData=null;
                                console.log("remove h5 hot zone", page_index, layer_index)
                            }
        ...
        get() {
            return this._INITIAL_STATE; // 返回存储的值
        },
