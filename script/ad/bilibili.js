...
/**
 * @typedef {Object} bilibili.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} bilibili.HTTPCallback
...
/**
 * @typedef {function(Object, bilibili.HTTPCallback): bilibili.HTTPResponse} bilibili.HTTPMethod
 */

/**
 * @typedef {Object} bilibili.HttpClient
 * @property {bilibili.HTTPMethod} get - 发送 GET 请求
 * @property {bilibili.HTTPMethod} post - 发送 POST 请求
 * @property {bilibili.HTTPMethod} put - 发送 PUT 请求
 * @property {bilibili.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {bilibili.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<bilibili.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {bilibili.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<bilibili.HTTPResponse>}
 */
...
function runtimeCode() {
    // 代码来源: https://greasyfork.org/zh-CN/scripts/467427-bilibili-%E8%87%AA%E5%8A%A8%E5%88%87%E6%8D%A2%E7%9B%B4%E6%92%AD%E7%94%BB%E8%B4%A8%E8%87%B3%E6%9C%80%E9%AB%98%E7%94%BB%E8%B4%A8/code
    function switchQuality() {
        const url = new URL(window.location.href)
        if (url.host === 'live.bilibili.com' && /\d+/g.test(url.pathname)) {
            let timer = setInterval(() => {

                // @ts-ignore
                if (typeof window.livePlayer !== "undefined") {
                    // @ts-ignore
                    const livePlayer = window.livePlayer
                    console.log("可选画质列表: ")

                    for (const q of livePlayer.getPlayerInfo().qualityCandidates) {
                        console.log(q)
                    }

                    if (livePlayer.getPlayerInfo().quality !== livePlayer.getPlayerInfo().qualityCandidates[0].qn) {
                        // @ts-ignore
                        livePlayer.switchQuality(livePlayer.getPlayerInfo().qualityCandidates[0].qn)
                        console.log(`切换画质到${livePlayer.getPlayerInfo().qualityCandidates[0].desc}`)
                    } else {
                        console.log(`已经是最高画质${livePlayer.getPlayerInfo().qualityCandidates[0].desc}`)
                    }
                    clearInterval(timer)
                }
            }, 100);
