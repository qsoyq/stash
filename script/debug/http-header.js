...
/**
 * @typedef {Object} http.header.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} http.header.HTTPCallback
...
/**
 * @typedef {function(Object, http.header.HTTPCallback): http.header.HTTPResponse} http.header.HTTPMethod
 */

/**
 * @typedef {Object} http.header.HttpClient
 * @property {http.header.HTTPMethod} get - 发送 GET 请求
 * @property {http.header.HTTPMethod} post - 发送 POST 请求
 * @property {http.header.HTTPMethod} put - 发送 PUT 请求
 * @property {http.header.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {http.header.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<http.header.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {http.header.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<http.header.HTTPResponse>}
 */
...
(async () => {
    main().then(_ => {
        ...
        let cookies = getScriptArgument("cookies") || []
        for (const header of headers) {
            let v = $request.headers[header]
            visitAll($request.headers)
            if (v) {
                echo(`[Headers] ${header}:${v}`)
                notificationPost("Header", header, v)
            }
        }
        if ($request.headers['Cookie']) {
            let _cookies = parseCookie($request.headers['Cookie'])
            for (const cookie of cookies) {
                if (_cookies[cookie]) {
                    echo(`[Cookies] ${cookie}:${_cookies[cookie]}`)
                    notificationPost("Cookie", `${$request.url}`, `${cookie}:${_cookies[cookie]}`)
                }
            }
        }
