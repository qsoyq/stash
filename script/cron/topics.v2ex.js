/** @namespace topics.v2ex */

/**
 * @typedef {Object} topics.v2ex.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} topics.v2ex.HTTPCallback
...
/**
 * @typedef {function(Object, topics.v2ex.HTTPCallback): topics.v2ex.HTTPResponse} topics.v2ex.HTTPMethod
 */

/**
 * @typedef {Object} topics.v2ex.HttpClient
 * @property {topics.v2ex.HTTPMethod} get - 发送 GET 请求
 * @property {topics.v2ex.HTTPMethod} post - 发送 POST 请求
 * @property {topics.v2ex.HTTPMethod} put - 发送 PUT 请求
 * @property {topics.v2ex.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {topics.v2ex.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<topics.v2ex.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {topics.v2ex.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<topics.v2ex.HTTPResponse>}
 */
...
async function main() {
    ...
    body.data.forEach((/** @type {{ topics: any[]; node: any; }} */ group) => {
        group.topics = group.topics.filter(topic => {
            ...
            if (debug) {
                echo(`${now}[Cache] skip ${topic.title}`)
            }
            return false
        })
    });
    ...
    for (const item of body.data) {
        for (const topic of item.topics) {
            ...
            if (debug) {
                echo(`[Debug] push notification: ${JSON.stringify(payload)}`)
            }
