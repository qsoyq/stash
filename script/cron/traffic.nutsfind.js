/** @namespace traffic.nutsfind */

/**
 * @typedef {Object} traffic.nutsfind.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} traffic.nutsfind.HTTPCallback
...
/**
 * @typedef {function(Object, traffic.nutsfind.HTTPCallback): traffic.nutsfind.HTTPResponse} traffic.nutsfind.HTTPMethod
 */

/**
 * @typedef {Object} traffic.nutsfind.HttpClient
 * @property {traffic.nutsfind.HTTPMethod} get - 发送 GET 请求
 * @property {traffic.nutsfind.HTTPMethod} post - 发送 POST 请求
 * @property {traffic.nutsfind.HTTPMethod} put - 发送 PUT 请求
 * @property {traffic.nutsfind.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {traffic.nutsfind.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<traffic.nutsfind.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {traffic.nutsfind.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<traffic.nutsfind.HTTPResponse>}
 */
...
async function trafficCron() {
    let decreaseTrafficAmountTrigger = getScriptArgument("decreaseTrafficAmountTrigger") || 10
    let authorization = getAuthorizationCache()?.authorization
    let cache = getUsedTrafficCache()
    let lastUsedTrafficTimestamp = cache?.lastUsedTrafficTimestamp
    let lastUsedTraffic = cache?.lastUsedTraffic
    if (!authorization) {
        echo("需要手动在网站上进行一次登陆以获取认证信息")
        return
    }

    echo(`lastBalance: ${lastUsedTraffic}, lastBalanceTimestamp: ${lastUsedTrafficTimestamp}`)
    let url = 'https://nutsfind.com/api/v1/user/getSubscribe'
    let headers = { "Authorization": authorization, }
    let res = await get({ url, headers })
    if (res.error || res.response.status >= 400) {
        echo(`[Error] request error: ${res.error}, ${res.response.status}, ${res.data}`)
        echo("获取订阅信息失败")
        notificationPost("Nutsfind", "Subscribe", "获取订阅信息失败")
        return
    }
    let body = parseJsonBody(res.data)
    let upload = body?.data?.u
    let download = body?.data?.d
    if (!upload || !download) {
        echo("获取使用流量数据失败")
        notificationPost("Nutsfind", "Traffic", "获取使用流量数据失败")
        return
    }
    let usedTraffic = (upload + download) / 1024 / 1024 / 1024 // GB
    usedTraffic = Number(usedTraffic.toFixed(3))
    let current = new Date().getTime() / 1000
    // 首次记录使用流量、流量重置(使用流量小于上次记录)、流量使用距离上次记录触发阈值
    // 针对三种情况通知提醒并记录日志
    if (!lastUsedTraffic) {
        echo(`当前已使用流量: ${usedTraffic} GB`)
        notificationPost("Nutsfind", "Traffic", `当前已使用流量: ${usedTraffic} GB`)
        updateUsedTrafficCache(usedTraffic, current)
        return
    } else if (usedTraffic < lastUsedTraffic) {
        echo(`流量重置, 当前已使用流量: ${usedTraffic} GB`)
        notificationPost("Nutsfind", "Traffic", `流量重置, 当前已使用流量: ${usedTraffic} GB`)
        updateUsedTrafficCache(usedTraffic, current)
        return
    } else {
        let decrease = usedTraffic - lastUsedTraffic
        let last = getLocalDateString(new Date(lastUsedTrafficTimestamp * 1000))
        // 流量
        if (decrease >= decreaseTrafficAmountTrigger) {
            let msg = `流量使用触发提醒\n当前使用流量: ${usedTraffic} GB\n上次使用流量记录: ${lastUsedTraffic} GB\n上次记录时间: ${last}`
            echo(msg)
            notificationPost("Nutsfind", "Traffic", msg)
            updateUsedTrafficCache(usedTraffic, current)
        }
    }
}
...
// https://nutsfind.com/api/v1/user/info
async function jwtRequest() {
    let authorization = $request.headers["Authorization"]
    echo(`[Traffic.Request] Authorization: ${authorization}`)
    if (authorization) {
        updateAuthorizationCache(authorization)
    }
}

function updateUsedTrafficCache(lastUsedTraffic, lastUsedTrafficTimestamp) {
    let persistentKeyName = 'used.traffic.nutsfind'
    let body = {}
    body.lastUsedTraffic = lastUsedTraffic
    body.lastUsedTrafficTimestamp = lastUsedTrafficTimestamp
    writePersistentArgument(persistentKeyName, JSON.stringify(body))
}

function updateAuthorizationCache(authorization) {
    let persistentKeyName = 'authorization.nutsfind'
    let body = {}
    body.authorization = authorization
    writePersistentArgument(persistentKeyName, JSON.stringify(body))
}

function getUsedTrafficCache() {
    let persistentKeyName = 'used.traffic.nutsfind'
    let cache = getPersistentArgument(persistentKeyName)
    if (!cache) {
        return {}
    }
    return parseJsonBody(cache) || {}
}

function getAuthorizationCache() {
    let persistentKeyName = 'authorization.nutsfind'
    let cache = getPersistentArgument(persistentKeyName)
    if (!cache) {
        return {}
