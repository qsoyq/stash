/** @namespace balance.nutsfind */

/**
 * @typedef {Object} balance.nutsfind.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} balance.nutsfind.HTTPCallback
...
/**
 * @typedef {function(Object, balance.nutsfind.HTTPCallback): balance.nutsfind.HTTPResponse} balance.nutsfind.HTTPMethod
 */

/**
 * @typedef {Object} balance.nutsfind.HttpClient
 * @property {balance.nutsfind.HTTPMethod} get - 发送 GET 请求
 * @property {balance.nutsfind.HTTPMethod} post - 发送 POST 请求
 * @property {balance.nutsfind.HTTPMethod} put - 发送 PUT 请求
 * @property {balance.nutsfind.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {balance.nutsfind.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<balance.nutsfind.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {balance.nutsfind.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<balance.nutsfind.HTTPResponse>}
 */
...
async function balanceCron() {
    let decreaseAmountTrigger = getScriptArgument("decreaseAmountTrigger") || 100
    let authorization = getAuthorizationCache()?.authorization
    let cache = getBalanceCache()
    let lastBalance = cache?.lastBalance
    let lastBalanceTimestamp = cache?.lastBalanceTimestamp
    if (!authorization) {
    ...
    if (res.error || res.response.status >= 400) {
        ...
        echo("获取用户信息失败")
        notificationPost("Nutsfind", "Balance", "获取用户信息失败")
        return
    }
    ...
    if (!balance) {
        echo("获取余额数据失败")
        notificationPost("Nutsfind", "Balance", "获取余额数据失败")
        return
    }
    let current = new Date().getTime() / 1000
    if (!lastBalance) {
        echo(`当前余额: ¥${balance / 100}`)
        notificationPost("Nutsfind", "Balance", `当前余额: ¥${balance / 100}`)
        updateBalanceCache(balance, current)
        return
    } else {
        let last = getLocalDateString(new Date(lastBalanceTimestamp * 1000))
        if (balance === lastBalance) {
            echo(`余额无变化: ¥${balance / 100}, 上次余额记录日期: ${last}`)
            return
        }
        if (balance > lastBalance) {
            let msg = `余额增加: ¥${lastBalance / 100} -> ¥${balance / 100}\n上次余额记录日期: ${last}`
            echo(msg)
            notificationPost("Nutsfind", "Balance", msg)
            updateBalanceCache(balance, current)
        } else {
            let decrease = lastBalance - balance
            if (decrease >= decreaseAmountTrigger) {
                let msg = `¥${lastBalance / 100} -> ¥${balance / 100}\n上次余额记录日期: ${last}`
                echo(msg)
                notificationPost("Nutsfind", "Balance", msg)
                updateBalanceCache(balance, current)
            } else {
...
async function balanceRequest() {
    ...
    echo(`[Balance.Request] Authorization: ${authorization}`)
    if (authorization) {
        updateAuthorizationCache(authorization)
    }
}

function updateBalanceCache(lastBalance, lastBalanceTimestamp) {
    let persistentKeyName = 'balance.nutsfind'
    let body = {}
    body.lastBalance = lastBalance
    body.lastBalanceTimestamp = lastBalanceTimestamp
