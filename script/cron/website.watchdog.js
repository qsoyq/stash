/** @namespace website.watchdog */

/**
 * @typedef {Object} website.watchdog.HTTPResponse
...
/**
 * @typedef {function(Error|string|null, Object, string|null): void} website.watchdog.HTTPCallback
...
/**
 * @typedef {function(Object, website.watchdog.HTTPCallback): website.watchdog.HTTPResponse} website.watchdog.HTTPMethod
 */

/**
 * @typedef {Object} website.watchdog.HttpClient
 * @property {website.watchdog.HTTPMethod} get - 发送 GET 请求
 * @property {website.watchdog.HTTPMethod} post - 发送 POST 请求
 * @property {website.watchdog.HTTPMethod} put - 发送 PUT 请求
 * @property {website.watchdog.HTTPMethod} delete - 发送 DELETE 请求
 */

/** @type {website.watchdog.HttpClient} */
var $httpClient;
 ...
 * @returns {Promise<website.watchdog.HTTPResponse>} 返回一个 Promise，解析为包含 error、response 和 data 的对象
 * @throws {Error} 如果请求失败，Promise 会被拒绝并返回错误信息
 */
async function request(method, params) {
    return new Promise((resolve, reject) => {
        /** @type {website.watchdog.HTTPMethod} */
 ...
 * 请求封装
 * @param {object} params
 * @returns {Promise<website.watchdog.HTTPResponse>}
 */
...
async function main() {
    ...
    let endpoints = getScriptArgument("endpoints") || []
    let limit = getScriptArgument("limit") || 3
    for (const endpoint of endpoints) {
        let resp = await get(endpoint)
        let cacheKey = `website.watchdog.${encodeURIComponent(endpoint)}`
        if (resp.error || resp.response.status >= 400) {
            echo(`[Request Error] ${resp.error}, ${resp.response.status}, ${resp.data}`)

            let count = parseInt(getPersistentArgument(cacheKey) || "0")
            count++

            echo(`[Request] execute ${endpoint} failed, count: ${count}`)
            writePersistentArgument(cacheKey, String(count))
            if (count >= limit){
                notificationPost(`Website Watchdog`, `${endpoint}`, `failed count: ${count}`)
            }
        }else{
            echo(`[Request] execute ${endpoint} done.`)
            writePersistentArgument(cacheKey, "0")
        }
    }
}
